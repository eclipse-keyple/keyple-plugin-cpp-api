 ...
 * keyple::core::plugin::spi::reader::ReaderSpi::transmitApdu(byte[]) method will be called
 * periodically by the service when a card removal is expected. The card is considered removed when
 * the transmission fails.
 *
 * <p>The value returned by the getCardRemovalMonitoringSleepDuration() will be used as an
 * argument to Thread::sleep(long) between two calls to
