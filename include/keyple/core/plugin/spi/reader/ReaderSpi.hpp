...
public:
    ...
    /**
     * Validates the opening of the physical channel. Performs the actual opening if this has not
     * been done by the checkCardPresence() method. In all cases, memorizes the new state for the
     * operation of the isPhysicalChannelOpen() method. After executing this method, the reader is
     * able to send APDUs to the card.
     *
    ...
    /**
     * Tells the reader that card processing is complete and that the next step is to remove the
     ...
     * logical opening state of the physical channel and letting the removal procedure do the
     * closing.
     *
     * @throw ReaderIOException If the communication with the reader has failed.
     * @since 2.0.0
     ...
     * contact reader equipped with a physical insertion detector using a switch), or will
     * communicate with the card (in the case of contactless hunting). In the latter case, we can
     * consider that the physical channel has been opened (and therefore no longer needs to be
     * opened in the openPhysicalChannel() method).
     *
     ...
     */
    friend std::ostream&
    operator<<(std::ostream& os, const std::vector<std::shared_ptr<ReaderSpi>>& rss) {
        os << "READER_SPIS: {";
